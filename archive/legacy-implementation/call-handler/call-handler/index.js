...
const handler = async (event) => {
    ...
    try {
        ...
        if (eventType === 'call.speak.ended') {
            const callControlId = webhook.data.payload.call_control_id;
            const clientStateBase64 = webhook.data.payload.client_state;
            ...
            if (clientState.waiting_for_speak_end) {
                ...
                const voice = clientState.voice || 'Polly.Joanna';
        ...
        if (eventType === 'call.ai.gathered' || eventType === 'call.ai_gather.ended' || eventType === 'call.ai.user_speech') {
            ...
            if (!speechResult || (status && status !== 'valid')) {
                ...
                try {
                    ...
                    const nextClientState = {
                        ...
                        voice: clientState.voice || 'Polly.Joanna' // CRITICAL: Always preserve voice
                    };
            ...
            try {
                ...
                const nextClientState = {
                    ...
                    voice: clientState.voice || 'Polly.Joanna' // CRITICAL: Always preserve voice to prevent switching
                };
