...
const sesClient = new client_ses_1.SESClient({ region: sesRegion });
...
const handler = async (event) => {
    ...
    try {
        ...
        const bookingAlreadyMade = conversationHistory.some(msg => msg.role === 'assistant' && (msg.content.toLowerCase().includes("i've got you down") ||
...
CRITICAL - Use Real Business Information ONLY:
- If a customer asks for "your address", "the address", "where you are", "your location" - they want the BUSINESS address. Respond with: "${business.address || 'I don\'t have that information available right now. Let me have someone from our team call you back with that information.'}"
- If a customer asks for "your phone number", "the phone number", "how to reach you" - they want the BUSINESS phone. Respond with: "${business.phone || 'I don\'t have that information available right now. Let me have someone from our team call you back with that information.'}"
- If a customer asks for "your email", "the email address", "how to email you" - they want the BUSINESS email. Respond with: "${business.email || 'I don\'t have that information available right now. Let me have someone from our team call you back with that information.'}"
...
- ${currentStatusText}
- When someone asks "are you open?" or "are you open right now?", tell them: ${currentStatusText}
- When someone says "today" or "tonight", they mean ${today}
- Check operating hours for ${dayOfWeek} to see if we're open
...
Remember: You're having a real conversation with a real person. Be natural, friendly, and helpful. Don't sound like a robot reading a script.`;
        ...
        const hasBookingContext = conversationHistory.some(msg => bookingKeywords.some(keyword => msg.content.toLowerCase().includes(keyword)));
        // Only do intent analysis if booking keywords are present OR we're in a booking context
        let intentAnalysis = null;
        let combinedIntent = null; // Declare outside so it's available in all handlers
        if (hasBookingKeywords || hasBookingContext) {
            ...
            if (intentAnalysis.customer_name)
                rememberedInfo.customer_name = intentAnalysis.customer_name;
            if (intentAnalysis.customer_phone)
                rememberedInfo.customer_phone = intentAnalysis.customer_phone;
            if (intentAnalysis.customer_email)
                rememberedInfo.customer_email = intentAnalysis.customer_email;
            if (intentAnalysis.requested_date)
                rememberedInfo.requested_date = intentAnalysis.requested_date;
            if (intentAnalysis.requested_time)
                rememberedInfo.requested_time = intentAnalysis.requested_time;
        ...
        const confidenceThreshold = (business.notification_settings?.ai_confidence_threshold ?? 0.8);
        ...
        if (combinedIntent && combinedIntent.intent === 'booking' && combinedIntent.confidence >= confidenceThreshold) {
            ...
            const quoteNeededService = services.find(svc => svc.quote_needed && conversationText.includes(svc.name.toLowerCase()));
            ...
            if (customerName && validPhone && requestedDate && requestedTime && !combinedIntent.needs_message) {
                ...
                if (bookingAlreadyMade) {
                    ...
                    const response = await callOpenAI(`The customer just said "${speech_text}" but we already successfully booked their appointment. Acknowledge what they said but confirm the booking is already set. Be brief and friendly.`, systemPrompt, conversationHistory);
                    const updatedHistory = [
                        ...conversationHistory,
                        { role: 'user', content: speech_text },
                ...
                const bookingResult = await attemptBooking(business, combinedIntent, appointments || [], today, dayOfWeek);
                if (bookingResult.success) {
                    ...
                    if (checkError) {
                        ...
                        if (currentCount >= maxPerSlot) {
                            ...
                            const alternatives = findAlternativeSlots(bookingResult.appointment_time, business.operating_hours[dayOfWeek]?.open || '09:00', business.operating_hours[dayOfWeek]?.close || '17:00', business.appointment_slot_duration, bookingResult.appointment_date, currentAppointments || [], maxPerSlot);
                            ...
                            if (aptError) {
                                ...
                                const updatedHistory = [
                                    ...conversationHistory,
                                    { role: 'user', content: speech_text },
                                    { role: 'assistant', content: bookingResult.response },
                                ].slice(-20);
                                return {
                                    statusCode: 200,
                                    headers,
                        ...
                        if (bookingResult.alternatives && bookingResult.alternatives.length > 0) {
                            ...
                            for (const altTime of bookingResult.alternatives) {
                                ...
                                if (timePatterns.some(pattern => speechLower.includes(pattern))) {
                                    ...
                                    const confirmedBookingResult = await attemptBooking(business, { ...combinedIntent, requested_time: altTime }, appointments || [], today, dayOfWeek);
                                    if (confirmedBookingResult.success) {
                                        ...
                                        if (!aptError) {
                                            ...
                                            const updatedHistory = [
                                                ...conversationHistory,
                                                { role: 'user', content: speech_text },
                                                { role: 'assistant', content: confirmedBookingResult.response },
                                            ].slice(-50);
                                            return {
                                                statusCode: 200,
                                                headers,
                ...
                const response = await callOpenAI(responsePrompt, systemPrompt, conversationHistory);
                // Update conversation history
                const updatedHistory = [
                    ...conversationHistory,
                    { role: 'user', content: speech_text },
        ...
        const aiHasTakenMessage = conversationHistory.some(msg => msg.role === 'assistant' && (msg.content.toLowerCase().includes("i'll pass") ||
        ...
        if ((finalIntent && (finalIntent.needs_message || finalIntent.confidence < confidenceThreshold) && !isSimpleConfirmation) || wantsMessage) {
            ...
            const originalMessageRequest = conversationHistory.find(msg => msg.role === 'user' && messageKeywords.some(keyword => msg.content.toLowerCase().includes(keyword)))?.content || speech_text;
...
async function analyzeIntent(speech, systemPrompt, business, today, dayOfWeek, appointments, conversationHistory = [], rememberedInfo = {}) {
...
async function attemptBooking(business, intent, existingAppointments, today, dayOfWeek) {
    ...
    const existingAtTime = existingAppointments.filter(apt => apt.appointment_date === requestedDate && apt.appointment_time === requestedTime).length;
    ...
    const alternatives = findAlternativeSlots(requestedTime, hours.open, hours.close, slotDuration, requestedDate, existingAppointments, maxPerSlot);
    if (alternatives.length > 0) {
...
function findAlternativeSlots(requestedTime, openTime, closeTime, slotDuration, date, existingAppointments, maxPerSlot) {
    ...
    for (let offset = slotDuration; offset <= slotDuration * 3; offset += slotDuration) {
        ...
        if (altMinutes >= openMinutes) {
            ...
            const existingAtAlt = existingAppointments.filter(apt => apt.appointment_date === date && apt.appointment_time === altTime).length;
...
async function extractCustomerInfo(speech, systemPrompt, conversationHistory = []) {
    const conversationContext = conversationHistory.length > 0
...
async function callOpenAI(prompt, systemPrompt, conversationHistory = []) {
...
async function sendBookingNotification(business, intent, date, time, category = 'booking') {
    const notificationSettings = business.notification_settings || {};
...
async function sendMessageNotification(business, customerInfo, message, requestType, category = 'call_back') {
    const notificationSettings = business.notification_settings || {};
    const notificationMethods = notificationSettings.notification_methods || {};
    const methods = notificationMethods[category] || ['email', 'sms']; // Default to both if not configured
...
function getDateDaysLater(dateStr, days) {
    const date = new Date(dateStr + 'T00:00:00');
